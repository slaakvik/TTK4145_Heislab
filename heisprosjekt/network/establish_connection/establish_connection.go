...
func EstablishConnToSlaves(id string, port string, masterConnCh chan<- net.Conn, connectionsCh chan<- map[string]net.Conn,
	 						sendMasterCh chan string, isMasterCh chan bool) (net.Conn, error) {
	isMaster := false
	connections := make(map[string]net.Conn)
	buffer := make([]byte, 1024)
