...
func TransmitConn(port string, id string, masterIp string) (net.Conn, error) {
	fmt.Println("[TransmitConn] Nå er jeg inni og skal lage en addr")
	addr, err := net.ResolveTCPAddr("tcp", masterIp+":"+port) // dette er bare foreløpig. IP-som mates inn må være destinasjonen
	if err != nil {
		fmt.Println("Error resolving address:", err)
		return nil, err
	}
	fmt.Println("[TransmitConn] Nå er jeg inni og skal lage en connection")
	conn, err := net.Dial(CONN_TYPE, addr.String())
	if err != nil {
		fmt.Printf("[error] Failed to Dial: %v\n", err)
		return nil, err
	}
	fmt.Println("[TransmitConn] Nå er jeg inni og har laget en connection")
	fmt.Println("[TransmitConn] Nå er jeg inni og skal skrive til connection")
	conn.Write([]byte(id))
	fmt.Println("[TransmitConn] Nå er jeg inni, men skal til å gå ut")
	return conn, err
}
...
func ReceiveConn(id string, port string, masterConnCh chan<- net.Conn, connectionsCh chan<- map[string]net.Conn, sendMasterCh chan string, listenAccepted chan struct{}) (net.Conn, error) {
	fmt.Println("[ReceiveConn] Nå har jeg kommet meg inni, entrer for-loopen")
	for {
		fmt.Println("[ReceiveConn] Venter her til det kommer en master...")
		select {
		case master := <-sendMasterCh:
			if id == master {
				fmt.Printf("[ReceiveConn] Jeg er masteren: %v\n", master)
				...
				fmt.Println("[ReceiveConn] Fått meg en adresse å sende på nå")
				// Create listener
				...
				fmt.Printf("[ReceiveConn] Akkurat laget en lytter: %v\n", listener)
				defer listener.Close()

				fmt.Println("Server listening on", addr.String())

				connections := make(map[string]net.Conn)
				fmt.Println("[ReceiveConn] På vei inn i for-loopen som lytter")
				// Accept incoming connections
				buffer := make([]byte, 1024)
				for {
					fmt.Println("[ReceiveConn] Venter her til noen biter på")
					masterConn, err := listener.Accept()
					fmt.Println("[ReceiveConn] Noen bet på kroken!")
					if err != nil {
					...
					fmt.Printf("[ReceiveConn] Se på storfangsten: %v\n", id)

					fmt.Println("[ReceiveConn] Denne fisken skal i samlingen min")
					connections[id] = masterConn
					fmt.Printf("[ReceiveConn] Er den ikke fin nå: %v", connections)

					fmt.Println("[ReceiveConn] sender masterConn til masterConnCh")
					masterConnCh <- masterConn
					fmt.Println("[ReceiveConn] sender så connections til connectctionsCh")
