...
func main() {
	...
	go establish_connection.EstablishConnToSlaves(elevId, CONN_PORT, masterConnCh, connectionsCh, masterIdToEstablishConnToSlaves, isMasterCh1)
	go slave.AlertMaster(CONN_PORT, elevId, masterIdToAlertMasterCh, masterIdToSendAndReceiveToMasterCh, slaveConnCh)

	go single_elev.ButtonsAndRequests(CONN_PORT, elevId, elevUpdateRealtimeCh,
	...
	go single_elev.OrderExecution(CONN_PORT, elevId, elevUpdateRealtimeCh, drv_floors,
