...
func main() {
	...
	go peers.GetPeerUpdate(peerUpdateCh, SendMasterCh)
	...
	sendMapToSlavesTx := make(chan map[string]elevator.Elevator)
	...
	go establish_connection.ReceiveConn(eleviId, masterPort, connCh, connectionsCh, SendMasterCh)

	// Master

	go tcp.SendAndReceive(connCh, connectionsCh, sendMapToSlavesTx, getElevFromSlaveRx)
	// Slave
	go master.NotifyMaster(masterPort, eleviId, SendMasterCh, sendMyselfToMasterTx)

	fmt.Printf("Started!\n")
	go fsm.ButtonsAndRequests(masterPort, eleviId, isMaster, elevUpdateRealtimeCh, drv_buttons /*elevatorTx, */, sendMapToSlavesTx, getElevFromSlaveRx, mapOfElevsRx, newOrderCh, lightsCh, sendMyselfToMasterTx)
